...
public class FeSimpleGeoProxBenchmarks {
	private class Results {
		...
		public Results(double radius) { this(radius, 0,0,0); }
	...
	Results timetest_bruteFirst (FeSimpleGeoProx<TestClientMapObject> world, LatLng start, double radius, int times) {
		...
		for (int ii=0; ii < times; ii++) {
			...
			Collection<MapClusterItem<TestClientMapObject>> pointsByBruteForce = FeSimpleGeoProxTest.searchByBruteForce(world, start, radius, LengthUnit.MILE);
			long t1=System.currentTimeMillis();
			Collection<MapClusterItem<TestClientMapObject>> pointsByWorldSearch = world.find (start, radius, LengthUnit.MILE);
	...
	public Given givenAWorld_with_1126656_points_nearAntiMeridian () {
		...
		Collection<MapClusterItem<TestClientMapObject>> mapObjects = FeSimpleGeoProxTest.make_many_points (southCenter.latitude, northCenter.latitude
				, -180.0, eastCenter.longitude);
		mapObjects.addAll( FeSimpleGeoProxTest.make_many_points (southCenter.latitude, northCenter.latitude
				, 177.0, westCenter.longitude));
		...
		System.out.println("Created test world with " + mapObjects.size() + " points in an area " 
				...
				+ " (" + eastCenter.longitude + " to " + westCenter.longitude + ")"
				+ ", and " 
				+ LatLngTool.distance(northCenter, southCenter, LengthUnit.MILE) + " miles north-to-south"
				+ " (" + northCenter.latitude + " to " + southCenter.latitude + ")"
	...
	String toRow (Given given, LatLng loc, Results results, int times) {
		...
		.append(given.eastCenter.longitude).append(separator)
		.append(given.westCenter.longitude).append(separator)
		.append(given.northCenter.latitude).append(separator)
		.append(given.southCenter.latitude).append(separator)
