...
public class LatLngUtils {
	...
	public static BoundingBox getBoundingBox(LatLng cornerNW, LatLng cornerSE) {
		...
		if ( crossesAntiMeridian(cornerNW, cornerSE)) {
			ret.add (new BoundingBoxlet.Builder()
				                            .setLongitudeWest(cornerNW.longitude) 
				                            .setLongitudeEast(180) 
				                            .setLatitudeNorth(cornerNW.latitude)
				                            .setLatitudeSouth(cornerSE.latitude)
				                            .build());
			ret.add (new BoundingBoxlet.Builder()
				                            .setLongitudeWest(-180) 
				                            .setLongitudeEast(cornerSE.longitude) 
				                            .setLatitudeNorth(cornerNW.latitude)
				                            .setLatitudeSouth(cornerSE.latitude)
				                            .build());
			
		} else { // just one box, from the smallest to the largest
			ret.add (new BoundingBoxlet.Builder().setLongitudeWest(cornerNW.longitude) 
				                                 .setLongitudeEast(cornerSE.longitude) 
				                                 .setLatitudeNorth(cornerNW.latitude)
				                                 .setLatitudeSouth(cornerSE.latitude) 
				                                 .build());
	...
	public static BoundingBox getBoundingBox(LatLng start, double radius, LengthUnit lengthUnit) {
		...
		double north = max(llNW.latitude, northCenter.latitude, llNE.latitude);
		double south = min(llSW.latitude, southCenter.latitude, llSE.latitude);
		...
		if ( crossesAntiMeridian(llNW, llNE) || crossesAntiMeridian(llSW, llSE)) { // There'll be two boxes, split across the antimeridian.  
			 ...
			 */
			double west = getSmallestPositive (llNW.longitude, westCenter.longitude, llSW.longitude);
			double east = getLargestNegative (llNE.longitude, eastCenter.longitude, llSE.longitude);
			...
			double west = min (westCenter.longitude, llNW.longitude, llSW.longitude);
			double east = max (eastCenter.longitude, llNE.longitude, llSE.longitude);
	...
	private static boolean crossesAntiMeridian (LatLng locWest, LatLng locEast) {
		return crossesAntiMeridian (locWest.longitude, locEast.longitude);
	}
	/**
